...
@section('content')
            ...
            <div class="col-12">
                ...
                <div class="card">
                    ...
                    <div class="mx-3" style="margin-top: 2rem;">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-4">
                                <div class="input-group input-group-static mb-3">
                                    ...
                                    <input type="date" class="form-control" value="{{ $record->tanggal }}" id="date"
                                        name="date" disabled>
                                </div>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                <div class="input-group input-group-static mb-3">
                                    ...
                                    <select class="form-control" name="shift" id="shiftSelector"
                                        onchange="onShiftChange()" disabled>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12">
                                <select class="form-control form-select" id="multiple_angkut" name="multiple_angkut[]"
                                    multiple disabled>
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-3 mt-4">
                                <div class="input-group input-group-static mb-3">
                                    <div class="input-group input-group-static mb-3">
                                        ...
                                        <input type="text" class="form-control" value="{{ $record->loading_point }}"
                                            name="loading_point" disabled>
                                    </div>
                                </div>
                            </div>
                            <div class="col-3 mt-4">
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <div class="form-actions">
                                        @if ($nik === $record->pengawas)
                                            <button type="button" class="btn btn-success btn-sm"
                                                onclick="approved('{{ $record->doc_num }}', '{{ $record->status }}', '{{ $nik }}', '{{ $record->pengawas }}')">
                                                <i class="fas fa-check"></i> Approve
                                            </button>
                                            <button type="button" class="btn btn-warning btn-sm"
                                                onclick="rejected('{{ $record->doc_num }}', '{{ $record->status }}', '{{ $nik }}', '{{ $record->pengawas }}')">
                                                <i class="fas fa-close"></i> Reject
                                            </button>
                                        @endif

                                        @if ($record->status === 'Rejected')
                                            @if ($nik == $record->checker)
                                                <button type="button" class="btn btn-primary btn-sm"
                                                    onclick="resetApproval('{{ $record->doc_num }}')">
    ...
    @section('custom-js')
        ...
        <script>
            ...
            function rejected(doc, status, nik, pengawas) {
                axios.post('{{ route('prod.form.checker.reject') }}', {
                        _token: "{{ csrf_token() }}",
                        doc_num: doc,
                        checked: nik == pengawas ? 'Rejected' : status,
                    })
            ...
            function approved(doc, status, nik, pengawas) {
                axios.post('{{ route('prod.form.checker.approve') }}', {
                        _token: "{{ csrf_token() }}",
                        doc_num: doc,
                        checked: nik == pengawas ? 'Approved' : status,
                    })
                    .then(function(response) {
                        console.log('Response:', response);
                        if (response.data.success) {
            ...
            function resetApproval(id) {
                if (confirm('Are you sure you want to reset this Approval?')) {
                    axios.post('{{ route('prod.form.checker.reset', ['id' => 'ID']) }}'.replace('ID', id))
                        .then(function(response) {
                            console.log('Response:', response);
                            if (response.data.success) {
                                Swal.fire({
    ...
    <td colspan="5" class="text-center">
        <select class="form-control form-select nama-operator-select" name="nama_operator_${alat}" id="nama_operator_${alat}" required style="background-color: #eee7e8; color: rgb(11, 10, 10); width: 100%;" disabled>
...
`;
                    ...
                    dataToShow.forEach((data, dataIndex) => {
                        ...
                        for (let i = 1; i <= 5; i++) {
                            ...
                            timeInputs +=
                                `<td><input type="time" class="form-control" name="${inputName}" value="${timeValue}" disabled></td>`;
                        }
                            ...
                            <div class="row input-row">
                                <div class="col-3 mt-4">
                                    <div class="input-group input-group-static mb-3">
                                        ...
                                        <input type="text" class="form-control" name="kendala_${alat}" value="${kendalaValue}" disabled>
                                    </div>
                                </div>
                                <div class="col-3 mt-4">
                                    <div class="input-group input-group-static mb-3">
                                        <label class="custom-text-color">Waktu Mulai</label>
                                        <input type="time" class="form-control" name="waktu_mulai_${alat}" value="${waktuMulaiValue}" disabled>
                                    </div>
                                </div>
                                <div class="col-3 mt-4">
                                    <div class="input-group input-group-static mb-3">
                                        <label class="custom-text-color">Waktu Selesai</label>
                                        <input type="time" class="form-control" name="waktu_selesai_${alat}" value="${waktuSelesaiValue}" disabled>
                                    </div>
                                </div>
                                <div class="col-3 mt-4">
                                    <div class="input-group input-group-static mb-3">
                                        <label class="custom-text-color">Keterangan</label>
                                        <input type="text" class="form-control" name="keterangan_${alat}" value="${keteranganValue}" disabled>
                                    </div>
                                </div>
                            </div>
                        </div>
