        ...
        <div class="card">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                    <h6 class="text-white text-capitalize ps-3">{{$isShowDetail ? 'Detail' : (isset($isEdit) && $isEdit ? 'Edit' : 'New')}} Maintenance CCTV</h6>
                </div>
            </div>
            
            <div class="card-body my-1">
                <form id="maintenanceForm" method="POST" action="{{ isset($isEdit) && $isEdit ? route('it-ops.update-cctv') : route('it-ops.submit-cctv') }}" class="form">
                    @csrf
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="mb-0 d-flex align-items-center">
                                        ...
                                        <input type="text" class="form-control input-text" id="area_cctv" name="area_cctv" required
                                            {{ $isShowDetail ? 'disabled' : '' }}
                                            value="{{ $isShowDetail || (isset($isEdit) && $isEdit) ? $maintenanceRecord->area_cctv : '' }}">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    @if(!$isShowDetail)
                        <div class="row mt-4">
                            <div class="col-12 d-flex justify-content-between align-items-center">
                                ...
                                <div>
                                    <button type="submit" class="btn btn-primary">{{ isset($isEdit) && $isEdit ? 'Update' : 'Submit' }}</button>
...
@section('custom-js')
    ...
    $(function() {
        ...
        maintenanceForm.submit(function(e) {
            ...
            @if(isset($isEdit) && $isEdit)
                var url = '{{ route("it-ops.update-cctv") }}';
            @else
                var url = '{{ route("it-ops.submit-cctv") }}';
            @endif
